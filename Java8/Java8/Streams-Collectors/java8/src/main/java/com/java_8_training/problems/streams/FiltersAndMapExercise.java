...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter(integer -> integer%2==0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(String -> String.equals("Jack")).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter(integer -> integer == null).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter(Person -> Person.getAge()==age && Person.getName().equals(name)).collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(integers -> integers.stream()).collect(toList());
    ...
    public static int findTheMinimumNumber(){
        Stream<Integer> input =  Stream.of(5, 2, 200, 33, 150, 20);
        //TODO #11
        // HINT: use reduce
        int min =0;
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(person -> person.getAge()).reduce(120, Integer::min);
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(integer -> Math.sqrt(integer) == (int)Math.sqrt(integer)).filter(integer -> integer%3==0).findFirst();
